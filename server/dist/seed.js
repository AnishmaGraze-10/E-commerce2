...
async function main() {
    ...
    const results = await Promise.all(curatedProducts.map((product) => Product.findOneAndUpdate({ name: product.name }, { $set: product }, { upsert: true, new: true, setDefaultsOnInsert: true })));
