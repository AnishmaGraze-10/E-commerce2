...
export default function ChatbotWidget() {
	...
	function generateDynamicBeautyResponse(input: string, context: UserContext): { text: string; type: 'text' | 'product' | 'routine' | 'ar' } {
		...
		if (lower.includes('skin') || lower.includes('analyze') || lower.includes('acne') || lower.includes('dark circles') || lower.includes('skin tone')) {
			return {
				text: "🔍 **Skin Analysis Ready!**\n\nI can analyze your skin using your camera to detect:\n• Skin tone and undertones\n• Common issues (acne, dark circles, dryness)\n• Personalized product recommendations\n\nClick 'Start Skin Analysis' to begin!",
				type: 'ar'
			}
		}
		
		// 2. Smart FAQ + Tutorial Videos
		if (lower.includes('how to apply') || lower.includes('tutorial') || lower.includes('how do i')) {
			const product = extractProductFromQuery(input)
			if (product && TUTORIAL_VIDEOS[product as keyof typeof TUTORIAL_VIDEOS]) {
				return {
					text: `💄 **How to Apply ${product.charAt(0).toUpperCase() + product.slice(1)}**\n\n**Step-by-Step Guide:**\n1. Prep your skin with moisturizer\n2. Use clean brushes or fingers\n3. Apply in thin layers\n4. Blend well for natural finish\n5. Set with setting spray\n\n**📹 Tutorial Video:** ${TUTORIAL_VIDEOS[product as keyof typeof TUTORIAL_VIDEOS]}`,
					type: 'text'
				}
			}
		}
		
		// 3. Personalized Beauty Routine Generator
		if (lower.includes('routine') || lower.includes('beauty routine') || lower.includes('skincare routine') || lower.includes('create routine')) {
			return {
				text: "📋 **Personalized Beauty Routine Generator**\n\nI'll ask you a few questions about your skin type, concerns, and lifestyle to create the perfect routine for you!\n\nLet's start with your skin type...",
				type: 'routine'
			}
		}
		
		// 4. Shopping Integration
		if (lower.includes('like') || lower.includes('want') || lower.includes('shade') || lower.includes('product') || lower.includes('buy') || lower.includes('purchase')) {
			const product = findProductInQuery(input)
			if (product) {
				return {
					text: `🛍️ **Found: ${product.name}**\n\n**Price:** ${product.price}\n**Category:** ${product.category}\n\nPerfect choice! This product will look great on you. Click 'Add to Cart' to purchase!`,
					type: 'product'
				}
			}
		}
		
		// 5. Shade & Product Recommendation Engine
		if (lower.includes('recommend') || lower.includes('suggest') || lower.includes('what should i use') || lower.includes('best')) {
			return {
				text: "✨ **Personalized Product Recommendations**\n\nI can suggest the perfect products based on:\n• Your skin tone (detected via camera)\n• Your preferred finish (matte, glossy, shimmer)\n• Your budget range\n• Your skin concerns\n\nTell me what you're looking for!",
				type: 'product'
			}
		}
		
		// 6. Camera Integration for Virtual Try-On
		if (lower.includes('try on') || lower.includes('ar') || lower.includes('virtual') || lower.includes('camera')) {
			return {
				text: "🎨 **Virtual Try-On with AR!**\n\nI can help you test:\n• 💋 Lipstick shades\n• 👁️ Eyeshadow looks\n• 🎭 Foundation matching\n• ✨ Highlighter placement\n\nClick 'Open AR Mode' to start your virtual makeover!",
				type: 'ar'
			}
		}
		
		// 7. Mood-Based Suggestions
		if (lower.includes('mood') || lower.includes('feeling') || lower.includes('occasion') || lower.includes('party') || lower.includes('office') || lower.includes('wedding')) {
			const mood = lower.includes('party') ? 'party' : lower.includes('office') ? 'office' : lower.includes('wedding') ? 'wedding' : 'casual'
		...
		if (lower.includes('help') || lower.includes('advice') || lower.includes('tips')) {
			return {
				text: "💄 **I'm your personal beauty advisor!** ✨\n\nI can help you with:\n• 🔍 Skin analysis & recommendations\n• 📚 Product tutorials & tips\n• 🛍️ Shopping & product suggestions\n• 🎨 Virtual try-ons with AR\n• 📋 Personalized beauty routines\n• 💫 Mood-based makeup looks\n\nWhat would you like to explore today?",
				type: 'text'
			}
		}
		
		// Product recommendations
		if (lower.includes('lipstick') || lower.includes('foundation') || lower.includes('eyeshadow') || lower.includes('skincare')) {
			const category = lower.includes('lipstick') ? 'lipstick' : lower.includes('foundation') ? 'foundation' : lower.includes('eyeshadow') ? 'eyeshadow' : 'skincare'
			const products = SAMPLE_PRODUCTS.filter(p => p.category === category)
			return {
				text: `Here are my top ${category} recommendations: ${products.map(p => `${p.name} (${p.price})`).join(', ')}. Would you like to try them with AR or add to cart?`,
				type: 'product'
			}
		}
		
		// Beauty routine
		if (lower.includes('routine') || lower.includes('daily') || lower.includes('skincare routine')) {
			return {
				text: "I'd love to create a personalized beauty routine for you! Tell me about your skin type (oily, dry, combination, sensitive) and your goals (anti-aging, acne treatment, hydration).",
				type: 'routine'
			}
		}
		...
		// Default responses
		const responses = [
			"Hi! I'm your AI Beauty Assistant! 💄 I can help with skin analysis, product recommendations, beauty routines, and virtual try-ons. What would you like to know?",
			"Hello! ✨ I'm here to help you with all your beauty needs. Try asking me about skin analysis, product tutorials, or beauty routines!",
			"Hey there! 🌸 I can assist with skincare advice, makeup tips, product recommendations, and even virtual try-ons. What can I help you with today?"
		]
